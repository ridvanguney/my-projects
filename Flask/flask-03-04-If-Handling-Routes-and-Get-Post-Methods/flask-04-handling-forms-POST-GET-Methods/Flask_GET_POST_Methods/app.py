...
def calculate():
    if request.method == "POST":
        ...
        return render_template("result.html", result1 = num1, result2 = num2, lcm = lcm(int(num1),int(num2)), developer_name = 'Ridvan')
    else:
        return render_template("result.html", developer_name = "Ridvan")

# Add a statement to run the Flask application which can be debugged.
if __name__== "__main__":
    # app.run(debug=True)
    app.aborter(host='0.0.0.0', post=80)